...
float *Layer::get_grad_in_batch()
{
    return grad_in_batch;
}

void Layer::set_grad_in_batch(float *grad_in)
{
    grad_in_batch = grad_in;
}
...
void Dense::backward_pass(float learning_rate)
{
    ...
    CUBLAS_CALL(cublasSaxpy(cublasHandle, n_weights, &eta, grad_weights, 1, weights, 1));

    // TODO (set 5): biases = biases + eta * grad_biases
    CUBLAS_CALL(cublasSaxpy(cublasHandle, n_biases, &eta, grad_biases, 1, biases, 1));
}
...
void Activation::forward_pass()
{
    ...
    CUDNN_CALL(cudnnActivationForward(cudnnHandle, 
    activation_desc, 
    &one, 
    in_shape, in_batch,
    &zero, 
...
void Conv2D::forward_pass()
{
    ...
    // std::cout << "Input shape: " << in_shape << std::endl;
    // std::cout << "Output shape: " << in_batch << std::endl;
    // std::cout << "filter dec: " << filter_desc << std::endl;
    // std::cout << "weights: " << in_batch << std::endl;
    // std::cout << "out_shape: " << in_shape << std::endl;
    // std::cout << "out_batch: " << in_batch << std::endl;
    ...
    std::cout << "YAY YAY du du du du: " << in_batch << std::endl;
...
ResidualBlock::ResidualBlock(Layer *prev, int n_kernels, int kernel_size, int stride, int padding,
    cublasHandle_t cublasHandle, cudnnHandle_t cudnnHandle)
: Layer(prev, cublasHandle, cudnnHandle)
{
    cudnnDataType_t dtype;
    ...
    int output_size = n * output_width * output_width;

    // std::cout << "YAY YAY du du 1: " << in_batch << std::endl;
    conv1 = new Conv2D(prev, n_kernels, kernel_size, stride, cublasHandle, cudnnHandle);
    relu1 = new Activation(conv1, CUDNN_ACTIVATION_RELU, 0.0, cublasHandle, cudnnHandle);

    // int skip_filters = prev->get_output_fwd().size() / (input_width * input_width);
    int skip_filters = n;
    int conv2_padding = (kernel_size - 1) / 2;

    // std::cout << "YAY YAY du du OKA 2: " << in_batch << std::endl;
    // conv2 = new Conv2D(relu1, skip_filters, kernel_size, 1, conv2_padding);
    conv2 = new Conv2D(relu1, n_kernels, kernel_size, 1, cublasHandle, cudnnHandle);
    // std::cout << "YAY YAY du du OKAY: " << in_batch << std::endl;
    relu2 = new Activation(conv2, CUDNN_ACTIVATION_RELU, 0.0, cublasHandle, cudnnHandle);

    if (stride != 1 || prev->get_output_fwd() != conv2->get_output_fwd()) {
        // skip_conv = new Conv2D(prev, skip_filters, 1, stride, 0);
        // std::cout << "YAY YAY du du du IF Here: " << in_batch << std::endl;
        skip_conv = new Conv2D(prev, skip_filters, 1, stride, cublasHandle, cudnnHandle);
    }
    ...
    CUDNN_CALL( cudnnSetTensor4dDescriptor(out_shape, CUDNN_TENSOR_NCHW,
        CUDNN_DATA_FLOAT, n, out_size, h, w) ); //or 1, 1


    // outpu.resize(conv2->get_output_fwd());
    // grad_input.resize(prev->get_output_fwd());
    allocate_buffers();
...
void ResidualBlock::forward_pass() {
    ...
    // Add skip connection
    if (skip_conv != nullptr) {
        // skip_conv->set_output(prev->get_output_fwd());
        
        // output[i] = conv2->get_output()[i]
        CUDA_CALL(cudaMemcpy(out_batch, conv2->get_output_fwd(), out_size * sizeof(float), cudaMemcpyDeviceToDevice));

        //do the skip
        skip_conv->forward_pass();

        //add the out_batches
        CUBLAS_CALL(cublasSaxpy(cublasHandle, out_size , &alpha, skip_conv->get_output_fwd(), 1, out_batch, 1));
        
        // std::cout << "ResidualBlock forward - Skip conv output size: " << skip_conv->get_output().size() << std::endl;

        // for (size_t i = 0; i < conv2->get_output().size(); ++i) {
        //     output[i] = conv2->get_output()[i] + skip_conv->get_output()[i];
        // }
        ...
        CUBLAS_CALL(cublasSaxpy(cublasHandle, out_size , &alpha, prev->get_output_fwd(), 1, out_batch, 1));
        // for (size_t i = 0; i < conv2->get_output().size(); ++i) {
        //     output[i] = conv2->get_output()[i] + prev->get_output()[i];
        // }
    }

    // relu2->set_output(output);
    relu2->forward_pass();
}

void ResidualBlock::backward_pass (float learning_rate) {
    relu2->backward_pass(learning_rate);

    // Backpropagate through skip connection
    if (skip_conv != nullptr) {

        CUDA_CALL(cudaMemcpy(grad_in_batch, relu2->get_grad_in_batch(), 
            batch_size * in_size * sizeof(float), cudaMemcpyDeviceToDevice));
        // for (size_t i = 0; i < grad_input.size(); ++i) {
        //     grad_input[i] = relu2->get_grad_input()[i];
        // }
        skip_conv->set_grad_in_batch(grad_in_batch);
        skip_conv->backward_pass(learning_rate);
    } else {
        CUDA_CALL(cudaMemcpy(grad_in_batch, relu2->get_grad_in_batch(), 
        batch_size * in_size * sizeof(float), cudaMemcpyDeviceToDevice));
        // for (size_t i = 0; i < grad_input.size(); ++i) {
    ...
    conv2->set_grad_in_batch(relu2->get_grad_in_batch());
    conv2->backward_pass(learning_rate);
    relu1->backward_pass(learning_rate);
    conv1->backward_pass(learning_rate);

    prev->set_grad_in_batch(conv1->get_grad_in_batch());
}
// // ResidualBlock class
// ResidualBlock::ResidualBlock(Layer *prev, int num_filters, int kernel_size,
//                            int stride, int padding, cublasHandle_t cublasHandle,
//                            cudnnHandle_t cudnnHandle)
//     : Layer(prev),
//       conv1(new Conv2D(prev, num_filters, kernel_size, stride, padding,
//                       cublasHandle, cudnnHandle)),
//       relu1(new Activation(conv1, Activation::ReLU, cudnnHandle)),
//       conv2(new Conv2D(relu1, num_filters, kernel_size, 1, padding, cublasHandle,
//                       cudnnHandle)),
//       relu2(new Activation(conv2, Activation::ReLU, cudnnHandle)),
//       skip_conv(nullptr) {
//   if (stride != 1 || prev->get_output().size() != conv2->get_output().size()) {
//     skip_conv = new Conv2D(prev, num_filters, 1, stride, 0, cublasHandle,
//                           cudnnHandle);
//   }
// }
...
//     // Add skip connection output to the main path output
//     int size = conv2->get_output().size();
//     float alpha = 1.0f;
//     CUBLAS_CHECK(cublasSaxpy(cublasHandle, size, &alpha, skip_conv->get_output(),
//                               1, conv2->get_output(), 1));
//   } else {
//     // Add input directly to the main path output
//     int size = conv2->get_output().size();
//     float alpha = 1.0f;
//     CUBLAS_CHECK(cublasSaxpy(cublasHandle, size, &alpha, prev->get_output(), 1,
//                               conv2->get_output(), 1));
//   }

//   // Apply ReLU activation to the final output
//   relu2->set_output(conv2->get_output(), conv2->get_output().size());
//   relu2->forward();

//   // Set the output of the ResidualBlock
//   set_output(relu2->get_output(), relu2->get_output().size());
// }

// void ResidualBlock::backward(float learning_rate) {
//     // Backpropagate through the second ReLU activation
//     relu2->set_grad_input(get_grad_input(), get_grad_input().size());
//     relu2->backward(learning_rate);

//     // Backpropagate through the second convolution layer
//     conv2->set_grad_input(relu2->get_grad_input(), relu2->get_grad_input().size());
//     conv2->backward(learning_rate);

//     // Backpropagate through the first ReLU activation
//     relu1->set_grad_input(conv2->get_grad_input(), conv2->get_grad_input().size());
//     relu1->backward(learning_rate);

//     // Backpropagate through the first convolution layer
//     conv1->set_grad_input(relu1->get_grad_input(), relu1->get_grad_input().size());
//     conv1->backward(learning_rate);

//     // Backpropagate through the skip connection (if it exists)
//     if (skip_conv != nullptr) {
//         skip_conv->set_grad_input(get_grad_input(), get_grad_input().size());
//         skip_conv->backward(learning_rate);

//         // Add the gradients from the skip connection to the gradients of the input
//         int size = get_grad_input().size();
//         float alpha = 1.0f;
//         CUBLAS_CHECK(cublasSaxpy(cublasHandle, size, &alpha, skip_conv->get_grad_input(), 1,
//                                 prev->get_grad_input(), 1));
//     } else {
//         // If there is no skip connection, the gradients of the input are the same as the gradients of the second convolution layer
//         int size = get_grad_input().size();
//         float alpha = 1.0f;
//         CUBLAS_CHECK(cublasScopy(cublasHandle, size, get_grad_input(), 1, prev->get_grad_input(), 1));
//     }
// }
/******************************************************************************/
...
//Its just the same as pool2D with  the mode parameter being CUDNN_POOLING_AVERAGE_COUNT_INCLUDE_PADDING

// GlobalAveragePooling::(Layer* prev, int stride, cudnnPoolingMode_t mode,
//     cublasHandle_t cublasHandle, cudnnHandle_t cudnnHandle)
// : Layer(prev, cublasHandle, cudnnHandle)
...
//     //find out ways to define num_channels
//     cudnnCreatePoolingDescriptor(&pooling_desc);

//     cudnnSetPooling2dDescriptor(poolingDesc, CUDNN_POOLING_AVERAGE_COUNT_INCLUDE_PADDING, CUDNN_PROPAGATE_NAN, input_width, input_width, 0, 0, 1, 1);
// }

//     CUDNN_CALL(cudnnSetTensor4dDescriptor(in_shape, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, n, c, h, w) );
//     CUDNN_CALL(cudnnSetTensor4dDescriptor(out_shape, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, n, c, h, w));
    
//     cudnnCreateTensorDescriptor(&outputDesc);

//     // int input_size = prev->get_output_fwd().size();
//     // int num_channels = batch_size / (std::sqrt(input_size) * std::sqrt(input_size));
...
// void GlobalAveragePooling::forward() {
//     const std::vector<float>& input = prev->get_output_fwd();
//     int input_width = std::sqrt(input.size());
//     int num_channels = input.size() / (input_width * input_width);

//     cudaMemcpy(d_input, input.data(), input.size() * sizeof(float), cudaMemcpyHostToDevice);

//     float alpha = 1.0f / (input_width * input_width);
//     float beta = 0.0f;

//     cudnnPoolingForward(cudnnHandle, poolingDesc, &alpha, inputDesc, d_input, &beta, outputDesc, d_output);
...
// void GlobalAveragePooling::backward(float /* learning_rate */) {
...
//     cudnnPoolingBackward(cudnnHandle, poolingDesc, &alpha, outputDesc, d_output, outputDesc, d_grad_input, inputDesc, d_input, &beta, inputDesc, d_grad_input);
...
void SoftmaxCrossEntropy::forward_pass()
{
    ...
    CUDNN_SOFTMAX_MODE_INSTANCE, &one, 
    in_shape, in_batch, 
    &zero, 
    out_shape, out_batch));
}
