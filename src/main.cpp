...
int main(int argc, char **argv)
{
    ...
    Model *model = new Model(64, c, h, w);
// #if DENSE
    // model->add("conv", { 16, 5, 1, 1});
    // model->add("conv", { 16, 3, 1 });
    // model->add(activation);
    // model->add("max pool", { 2 });

    // model->add("residual block", { 32, 3, 1, 1 }); //adjust for padding
    // model->add("max pool", { 2 });

    // model->add("global average pool", { 2 }); //check
    // model->add("dense", { 200 });
    ...
    model->add("global average pool", { 2 });
    // model->add("dense", { 200 });
    // model->add("softmax crossentropy");
